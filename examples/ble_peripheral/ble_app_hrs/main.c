...
static uint8 loraworkmodeTimeout;

uint8 loraworkmode;
uint8 loraWakeupmode;

extern uint8  ProtoAnaly_LoraBrgBuf[30];
extern uint8  ProtoAnaly_LoraBrgSize;


void LoraWakeup_timer_Start(uint8 lstate, uint16 time)
{	
	// 重新启动检测CAD 前将超时清除
	Sys_SpiOpen();
	RadioDrive.ioctl(RADIO_SET_SLEEP,NULL);
	Sys_SpiClose();
	app_timer_stop(LoraWakeup);
	app_timer_start(LoraWakeup, APP_TIMER_TICKS(time, APP_TIMER_PRESCALER), NULL);
	loraWakeupmode= lstate;
	loraworkmode= RADIO_SET_SLEEP;
}

void LoraRcing_timer_Delay(uint16 time)
{	
	loraworkmodeTimeout =0;
	loraworkmode= RADIO_SET_RX;
}
void LoraCallBacktimer_Start(uint16 time)
{		
	app_timer_stop(LoraCallBack);
	app_timer_start(LoraCallBack, APP_TIMER_TICKS(time, APP_TIMER_PRESCALER), NULL);
}
static void LoraWakeup_timer_handler(void * p_context)
{
	Sys_SpiOpen();
	if(ProtoAnaly_LoraBrgSize == 0)
	{
		ProtoAnaly_LoraBrgSize = ProtoAnaly_RecordGetCmd(ProtoAnaly_LoraBrgBuf);
		if(ProtoAnaly_LoraBrgSize != 0)
		{
			Proto_NetCommSend(LORA,ProtoAnaly_LoraBrgBuf,ProtoAnaly_LoraBrgSize);
			ProtoAnaly_LoraBrgSize = 0;
			loraworkmodeTimeout = 0;
			LoraCallBacktimer_Start(1);
			loraworkmode= RADIO_SET_TX;
		}
		else 
		{
			if(loraWakeupmode ==RADIO_SET_CAD_DET)
			{
				loraworkmode = RADIO_SET_CAD_DET;
				RadioDrive.ioctl(RADIO_SET_CAD_DET,NULL);
				Lora_CadCount = 0;
				LoraCallBacktimer_Start(1);
			}
			else if(loraWakeupmode ==RADIO_SET_RX)
			{
				RadioDrive.ioctl(RADIO_SET_RX,NULL);
				loraworkmode= RADIO_SET_RX;
				loraworkmodeTimeout = 0;
				LoraCallBacktimer_Start(1);
			}
...
static void LoraCallBack_handler(void * p_context)
{
	uint8 restate;
	
	Sys_SpiOpen();
	restate= RadioDrive.ioctl(RADIO_SCAN_PROC,NULL);
	if(loraworkmode == RADIO_SET_CAD_DET)
	{
		if(restate== RF_CHANNEL_ACTIVITY_DETECTED)
		{
			if(++Lora_CadCount>= 3)
			{
				// CAD检测到后 切换为接收
		...
		if(++loraworkmodeTimeout>= (PROTO_BREAD_TIME_TABLE[Sys_PataCfg.rfPara.Speed]))
		{
			loraworkmodeTimeout =0;
			LoraWakeup_timer_Start(RADIO_SET_CAD_DET,PROTO_SLEEP_TIME_TABLE[Sys_PataCfg.rfPara.Speed]);
		}
		LoraCallBacktimer_Start(1);
	}
	else if(loraworkmode == RADIO_SET_RX)
	{
		if(++loraworkmodeTimeout>= ((PROTOANALY_REC_WINDOWTIMES*(1<<Sys_PataCfg.rfPara.Speed) )*2 +5))
		{
			loraworkmodeTimeout =0;
			LoraWakeup_timer_Start(RADIO_SET_CAD_DET,PROTO_SLEEP_TIME_TABLE[Sys_PataCfg.rfPara.Speed]);
		}
	...
	else if(loraworkmode == RADIO_SET_TX)
	{
		if(restate == RF_TX_DONE)
		{
			loraworkmodeTimeout =0;
			ProtoAnaly_LoraBrgSize = ProtoAnaly_RecordGetCmd(ProtoAnaly_LoraBrgBuf);
			if(ProtoAnaly_LoraBrgSize != 0)
			{
				LoraWakeup_timer_Start(RADIO_SET_TX,(rand()&0x0f)*PROTOANALY_SEND_WINDOWTIMES*(1<<Sys_PataCfg.rfPara.Speed)+20);
			}
			else 
			{
				LoraWakeup_timer_Start(RADIO_SET_CAD_DET,PROTO_SLEEP_TIME_TABLE[Sys_PataCfg.rfPara.Speed]);
		...
		// 发送超时
		if(++loraworkmodeTimeout>=(PROTOANALY_SEND_WINDOWTIMES*(1<<Sys_PataCfg.rfPara.Speed) +20))
		{
			loraworkmodeTimeout =0;
			ProtoAnaly_LoraBrgSize = ProtoAnaly_RecordGetCmd(ProtoAnaly_LoraBrgBuf);
			if(ProtoAnaly_LoraBrgSize != 0)
...
#if (defined SUPPORT_DETDOOR) && (SUPPORT_DETDOOR == STD_TRUE)
	MotorDrive.ioctl(MOTOR_CMD_SCAN_DOOR, NULL);
#endif
#if (defined LORA_ENABLE) && (LORA_ENABLE == STD_TRUE)
	if(loraworkmode != RADIO_SET_SLEEP)	
	{
		Carddet_timer_start(100);
		return ;
	}
#endif
      // return;
...
// Create timers.
#if (defined LORA_ENABLE) && (LORA_ENABLE == STD_TRUE)
	err_code = app_timer_create(&LoraWakeup, APP_TIMER_MODE_SINGLE_SHOT, LoraWakeup_timer_handler);
	err_code = app_timer_create(&LoraCallBack, APP_TIMER_MODE_SINGLE_SHOT, LoraCallBack_handler);
#endif
...
{
	static uint8 sysbasewkflag = STD_FALSE;
...
void Lora_ReordSendProc(void)
{
	...
	if(ProtoAnaly_LoraBrgSize != 0)
	{
		sentime = (rand()%10)*50+100;
		LoraWakeup_timer_Start(RADIO_SET_TX,sentime);
	}
}
#endif
...
#if (defined LORA_ENABLE) && (LORA_ENABLE == STD_TRUE)	
	LoraWakeup_timer_Start(RADIO_SET_CAD_DET,5000);
#endif
	Battdet_timer_start(1000);
	Carddet_timer_start(1000);
	SysBasetimer_Start(64);
#if (defined WATCHDOG_EN) && (WATCHDOG_EN == STD_TRUE)
	Sys_wdt_start();
#endif
