...
{	
	...
	uint8 lenthhfbyte, currentMax, totalMax, keyType;
	...
	{	
		...
		for(uint8 j= 0;j< currentMax;)
		{	
			for(uint8 i =0;i <totalMax;i++)
			{
				if(idtpye == 0)
				{// 黑名单
			...
			else
			{
				...
				/*else if(idtpye == 2)
				{
					keyData = LockKeyAppKeyList.KeyList[i].PaswdId;
				}*/
				if(keyData == 0xffffffff)
				{
					continue;
...
uint8 LockKey_Check_CardKey(uint8 idtpye,uint8 *pid)
{
	uint8 index;
	uint8 temp_buf[8];
	uint32 keyId, keyData, startTime, endTime;
	uint8 ret=E_NOT_OK;
