...
void Access_Init(void)
{
	...
	//SensorDrive.open(NULL);
#else
	/*nrf_gpio_cfg_output(DET_REVERSELOCKSW);
	ReverseLock_Low();
	nrf_gpio_cfg_output(DET_FALSELOCKSW);
	FalseLock_Low();
	nrf_gpio_cfg_output(DET_MECHANICALKEYSW);
	MechanicalKey_Low();*/
#endif
...
void Access_Unlock(void)
{
	...
	else if( Access_BatteryData<Batt_VolPerTable[BATT_VOLPER_MAX-3][1] )
	{
		Access_BeepStart(BEEP_SLOW,BEEP_ERROR_LOW_VOL);
		Access_LightStart(LIGHT_LOW,LIGHT_SLOW,1,0);
...
void Access_WriteRecordFlash(uint8* pKeyId,uint32 time ,uint8 type, uint8 action)
{
	...
	tempbuf[0]= type;
	if(type == KEY_TYPE_IDENTITY_CARD)
	{
		memcpy(&tempbuf[1],pKeyId,8);
	}
	else if(type == KEY_TYPE_TIME_INDEX)
	{
		memcpy(&tempbuf[1],pKeyId,2);
	}   
	else 
	{
		memcpy(&tempbuf[1],pKeyId,4);
	}
	tempbuf[9]= action;
	Sys_GenFun32To8(time,&tempbuf[10]);
	if((AccRcord.Wrecordindex&0xff) == 0)
	{
		uint32 eraseadd;
		eraseadd = ACCESS_RECORD_OPEN_OFFSET+((AccRcord.Wrecordindex&(ACCESS_RECORD_MAX))<<4);	
		FlashDrive.ioctl(FLASH_ERASE_SECT,&eraseadd);
	}
	fdata.Add = ACCESS_RECORD_OPEN_OFFSET+((AccRcord.Wrecordindex&(ACCESS_RECORD_MAX))<<4);
	fdata.pData = tempbuf;
	fdata.Lenth = 14;
	FlashDrive.write(&fdata);
	if(AccRcord.recordnum<= ACCESS_RECORD_RDMAX)
	{
		if(AccRcord.recordnum== ACCESS_RECORD_RDMAX)
		{
			AccRcord.Strecordindex++;
		}
		AccRcord.recordnum++;
	}
	else 
	{
		AccRcord.Strecordindex++;
		if(AccRcord.Strecordindex >= ACCESS_RECORD_MAX)
		{
			AccRcord.Strecordindex = 0;
		}
		if(AccRcord.Rdtrecordindex != 0)
		{
			AccRcord.Rdtrecordindex--;
		}
	}
	AccRcord.Wrecordindex++;
	Access_Store_Recordx64ms = 2000/64;
#endif
}
/****************************************************************************************************
**Function:
