...
/*	#if (defined DETECT_SW_EN) && (DETECT_SW_EN == STD_TRUE)
		if((ReverseLockGetData()==0))
	       {
			SensorDrive.ioctl(SENSOR_CMD_CHECK, NULL);
	}
#endif
#if (defined MECHANICAL_KEY_DET_EN) && (MECHANICAL_KEY_DET_EN == STD_TRUE)
		if(MechanicalKeyGetData() == 0)
       {
			SensorDrive.ioctl(SENSOR_CMD_CHECK, NULL);
	}
#endif
